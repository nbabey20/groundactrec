...
def _eval_on_csv(model, csv_path, label2id, args):
    ...
    _eval_model = _wrap_for_eval(model, device, getattr(args, "fusion_type", "xattn"))
    return evaluate(_eval_model, loader, device, num_classes=len(label2id), amp=args.amp, label2id=label2id)
...
def run_occlusion_study(args):
    ...
    if args.fusion_type == "xattn":
        ...
        model = LateScoreAvgModel(
            d_video_in=d_video_in, d_skel_in=d_skel_in,
            d_model=d_model, n_heads=n_heads, depth=depth,
            ff_mult=ff_mult, dropout=dropout,
            num_classes=num_classes, T=T,
            fuse_on=args.late_fuse_on
        ).to(device)
    ...
    with open(out_dir / "occ_eval_fusion.md", "w") as f:
        ...
        f.write("|---|---:|---:|---:|\n")
        f.write(f"| Fusion (X-Attn) | {low_top1:.1f}% | {high_top1:.1f}% | {delta_pp:+.1f} |\n")
...
class FusionXAttnModel(nn.Module):
    ...
    # Inside the LateScoreAvgModel class in your script...

    def forward(self, V, S):
        logits_v = self._branch(V, self.v_proj, self.v_cls, self.v_pos, self.v_enc, self.v_norm, self.v_head)
        logits_s = self._branch(S, self.s_proj, self.s_cls, self.s_pos, self.s_enc, self.s_norm, self.s_head)
    
        if self.fuse_on == "probs":
            p = (torch.softmax(logits_v, dim=-1) + torch.softmax(logits_s, dim=-1)) * 0.5
            fused = torch.log(torch.clamp(p, 1e-8, 1.0))
        else: # 'logits'
            fused = 0.5 * (logits_v + logits_s)
        
        # Ensure you are returning all three values
        return fused, logits_v, logits_s
...
class EarlyConcatFusionModel(nn.Module):
    """Early fusion: per-frame projection + concat, single temporal encoder, one CLS."""
    def __init__(self, d_video_in=1408, d_skel_in=85, d_model=512, n_heads=8, depth=4, ff_mult=4,
                 dropout=0.1, num_classes=14, T=64):
        super().__init__()
        self.v_proj = nn.Linear(d_video_in, d_model // 2)
        self.s_proj = nn.Linear(d_skel_in,  d_model // 2)
    ...
    def forward(self, V, S):
        v = self.v_proj(V)        # [B,T,d_model/2]
        s = self.s_proj(S)        # [B,T,d_model/2]
        x = torch.cat([v, s], dim=-1)  # [B,T,d_model]
        B, T, D = x.shape
        x = torch.cat([self.cls.expand(B,1,D), x], dim=1)
        x = self.pos(x)
        x = self.encoder(x)
        cls = self.norm(x[:,0])
...
class LateScoreAvgModel(nn.Module):
    """Late fusion: two unimodal temporal encoders; fuse by logit/prob avg."""
    def __init__(self, d_video_in=1408, d_skel_in=85, d_model=512, n_heads=8, depth=4, ff_mult=4,
                 dropout=0.1, num_classes=14, T=64, fuse_on="logits"):
        super().__init__()
        # video branch
        self.v_proj = nn.Linear(d_video_in, d_model)
        self.v_cls = nn.Parameter(torch.zeros(1,1,d_model))
        self.v_pos = PositionalEncoding(d_model, max_len=T+1)
        v_layer = nn.TransformerEncoderLayer(
            d_model=d_model, nhead=n_heads, dim_feedforward=d_model*ff_mult,
            dropout=dropout, batch_first=True, activation="gelu", norm_first=True
        )
        self.v_enc = nn.TransformerEncoder(v_layer, num_layers=depth)
        self.v_norm = nn.LayerNorm(d_model)
        self.v_head = nn.Linear(d_model, num_classes)

        # skeleton branch
        self.s_proj = nn.Linear(d_skel_in, d_model)
        self.s_cls = nn.Parameter(torch.zeros(1,1,d_model))
        self.s_pos = PositionalEncoding(d_model, max_len=T+1)
        s_layer = nn.TransformerEncoderLayer(
            d_model=d_model, nhead=n_heads, dim_feedforward=d_model*ff_mult,
            dropout=dropout, batch_first=True, activation="gelu", norm_first=True
        )
        self.s_enc = nn.TransformerEncoder(s_layer, num_layers=depth)
        self.s_norm = nn.LayerNorm(d_model)
        self.s_head = nn.Linear(d_model, num_classes)

        self.fuse_on = fuse_on  # "logits" or "probs"
        nn.init.normal_(self.v_cls, std=0.02); nn.init.normal_(self.s_cls, std=0.02)

    def _branch(self, X, proj, cls, pos, enc, norm, head):
        x = proj(X)
        B,T,D = x.shape
        x = torch.cat([cls.expand(B,1,D), x], dim=1)
        x = pos(x)
        x = enc(x)
    ...
    def forward(self, V, S):
        ...
        if self.fuse_on == "probs":
            p = (torch.softmax(logits_v, dim=-1) + torch.softmax(logits_s, dim=-1)) * 0.5
            fused = torch.log(torch.clamp(p, 1e-8, 1.0))  # stabilized log-probs -> logits-ish for CE
        else:
            fused = 0.5 * (logits_v + logits_s)
        return fused, logits_v, logits_s
...
# Eval helpers
# ----------------------------
# --- evaluation wrapper for late fusion ---
class _LateEval(nn.Module):
    def __init__(self, m): 
        super().__init__(); self.m = m
    def forward(self, V, S):
        fused, _, _ = self.m(V, S)  # use fused logits for eval
        return fused

def _wrap_for_eval(model: nn.Module, device: str, fusion_type: str) -> nn.Module:
    """Return a logits-only model for evaluators."""
    if fusion_type == "late":
        return _LateEval(model).to(device)
    return model
...
def train(args):
    ...
    if args.fusion_type == "xattn":
        model = FusionXAttnModel(
            ...
            enable_self_attn=not args.disable_self_attn
        ).to(device)
    elif args.fusion_type == "early":
        model = EarlyConcatFusionModel(
            d_video_in=args.d_video_in, d_skel_in=d_skel_in,
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for V, S, y in pbar:
            ...
            with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=args.amp and torch.cuda.is_available()):
                if args.fusion_type == "late":
                    fused, logits_v, logits_s = model(V, S)
                    if args.late_train_mode == "multitask":
                        loss = 0.5 * (criterion(logits_v, y) + criterion(logits_s, y))
                    else:  # fused
        ...
        # Eval
        _eval_model = _wrap_for_eval(model, device, args.fusion_type)
        metrics = evaluate(_eval_model, val_loader, device, num_classes, amp=args.amp, label2id=label2id)
    ...
    _eval_model, val_loader, device, num_classes, label2id, out_dir, amp=args.amp
        )
        fps = measure_inference_fps(_eval_model, val_loader, device, amp=args.amp)
        ...
        model_name = {"xattn":"Fusion (X-Attn)", "early":"Fusion (Early-Concat)", "late":"Fusion (Late-Avg)"}[args.fusion_type]
        headline = (
            f"{model_name} | Top-1 {final_metrics['top1']*100:5.2f}% | "
...
if __name__ == "__main__":
    ...
    ap.add_argument("--fusion_type", choices=["xattn", "early", "late"], default="xattn",
                help="Which fusion baseline to train/eval.")
    ap.add_argument("--late_train_mode", choices=["fused", "multitask"], default="fused",
                help="Late fusion loss: 'fused' uses only fused logits; 'multitask' sums branch losses.")
    ap.add_argument("--late_fuse_on", choices=["logits", "probs"], default="logits", help="Fuse late fusion on raw 'logits' or normalized 'probs'.")
